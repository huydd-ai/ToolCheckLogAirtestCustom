...
def main():
    # TC13 -- With event active (nothing unclaimed), jump the device clock
    ...
    # the player finishes one more level (verified separately by tc15).
    # Test Flow:
    # Step 1: Check icon still visible immediately after reopen past expiry (no cue, no popup)
    #
    try:
        log_info("Start: tc13_event_end_finished")
        ...
        if not magic_bean.is_event_icon_visible(timeout=15):
            raise AssertionError(
                ...
                "(removal requires 1 more level, see tc15) | Actual: icon already gone"
            )
        log_info(
            "Result: Expected: icon still visible right after reopen past duration expiry | Actual: visible"
        )

        log_info("End: tc13_event_end_finished")
