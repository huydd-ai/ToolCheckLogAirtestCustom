...
def main():
    ...
    try:
        log_info("Start: tc16_collect_nails_red")
        ...
        target_task_type = "COLLECT_PIN"
        target_items = [
            "pin_red", "pin_blue", "pin_brown", "pin_yellow", 
            "pin_pink", "pin_cyan", "pin_gray", "pin_green", 
            "pin_orange", "pin_purple", "pin_light_blue", "pin_dark_blue"
        ]
        MAX_REROLLS = 25
        log_info(f"Start: find or reroll mission {target_task_type}")
        mission_data, found_item = daily.find_or_reroll_mission(
            target_task_type, target_items=target_items, home_page=home_page, max_rerolls=MAX_REROLLS
        )

        if not mission_data:
            raise AssertionError(f"No active mission of type {target_task_type} for any pin color after {MAX_REROLLS} rerolls!")
        ...
        sleep(10)
        log_info(f"End: tc16_collect_nails_red successfully tested [{mission_name}]")
