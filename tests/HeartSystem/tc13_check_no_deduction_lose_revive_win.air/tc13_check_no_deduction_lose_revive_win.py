...
def main():
    # STT 24 — Case 2 (no deduction): Lose → Revive → Win → no heart deducted
    try:
        log_info("Start: setup heart test (heart=5, level=12, coin=10000)")
        profile = {
            "fakeads": True,
            "heart": 5,
            "level": 12,
        ...
        log_info("Start: record heart count before game")
        count_before = heart_page.get_heart_count_via_ocr()
        log_info(f"Heart count before: {count_before}")
        log_info("End: record heart count before game")

        log_info("Start: enter game and trigger lose")
        run_step("click play", home_page.click_play)
        sleep(5)
        run_step("set level lose", set_param, "set_level_win", False)
        sleep(3)
        log_info("End: enter game and trigger lose")

        log_info("Start: tap revive (btn_buy_heart_by_coin)")
        if not home_page.tap(heart_page.btn_buy_heart_by_coin):
            wrapper.log_warning("Revive button not found, tapping close instead")
            run_step("tap close", home_page.tap, home_page.btn_close)
        sleep(2)
        log_info("End: tap revive")

        log_info("Start: win after revive")
        run_step("set level win", set_param, "set_level_win", True)
        sleep(3)
        run_step("tap next", home_page.tap, home_page.btn_next)
        sleep(2)
        run_step("navigate to home", go_home_clean, home_page)
        log_info("End: win after revive")

        log_info("Start: verify no heart deducted")
        count_after = heart_page.get_heart_count_via_ocr()
        log_info(f"Heart count after: {count_after}")
        if count_after != count_before:
            raise AssertionError(
                f"Heart deducted on revive+win: before={count_before}, after={count_after}"
            )
        else:
            log_info(f"Result: Expected count_after={count_before} | Actual count_after={count_after}")
        log_info("End: verify no heart deducted")
