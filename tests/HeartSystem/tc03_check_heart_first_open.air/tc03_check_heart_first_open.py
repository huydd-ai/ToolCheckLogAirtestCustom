...
def main():
    # STT 3 — Check heart count when opening game for the first time: expect Heart = 5
    try:
        ...
        log_info("Start: check heart count on first open")
        count = heart_page.get_heart_count_via_ocr()
        if count != 5:
            raise AssertionError(f"Expected heart=5 on first open, got {count}")
        else:
            log_info(f"Result: Expected count=5 | Actual count={count}")
        log_info("End: check heart count on first open")
