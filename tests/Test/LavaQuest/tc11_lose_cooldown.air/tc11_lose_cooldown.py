...
def main():
    try:
        log_info("Start: tc11_lose_cooldown")
        ...
        log_info("[Step 4] START: open LQ popup, verify cooldown active (btn_start hidden, timer visible)", snapshot=False)
        if not lava_quest.open_lava_quest_popup():
            raise AssertionError("Lava Quest popup did not open after lose")
        sleep(2)

        start_available = lava_quest.wait_for_element(lava_quest.btn_start, timeout=3)
        cooldown_visible = lava_quest.is_time_count_down_visible(timeout=5) or lava_quest.is_timer_running_in_zone(timeout=5)
        log_info(f"[Step 4] RESULT — expected: btn_start unavailable, cooldown timer visible | actual: start_available={bool(start_available)}, cooldown_visible={bool(cooldown_visible)}, timer_text={(lava_quest.check_timer_text() if cooldown_visible else '')!r}")
        if start_available:
            raise AssertionError("btn_start should NOT be available during 15-min cooldown")
        if not cooldown_visible:
            raise AssertionError("Cooldown timer NOT visible during 15-min wait")
        log_info("[Step 4] END", snapshot=False)

        home_page.tap(home_page.btn_close)
        sleep(1)

        log_info("[Step 5] START: advance +16min past cooldown, reopen, verify btn_start available", snapshot=False)
        keyevent("HOME")
        sleep(2)
        set_time_relative(16.0 / 60.0)
        sleep(10)
        run_step("resume hibernated app (no restart)", resume_app)
        run_step("close popups after time advance", close_all_popups, home_page)
        run_step("navigate home after time advance", go_home_clean, home_page)

        if not lava_quest.open_lava_quest_popup():
            raise AssertionError("Lava Quest popup did not open after cooldown expired")
        sleep(2)

        start_available_after = lava_quest.wait_for_element(lava_quest.btn_start, timeout=5)
        log_info(f"[Step 5] RESULT — expected: btn_start available after 15-min cooldown expired | actual: start_available={bool(start_available_after)}")
        if not start_available_after:
            raise AssertionError("btn_start NOT available after 15-min cooldown expired")
        log_info("[Step 5] END", snapshot=False)

        sleep(3)
        log_info("End: tc11_lose_cooldown — all checks passed")
